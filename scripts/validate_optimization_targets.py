...
def simulate_workload(num_tasks: int = 100):
    ...
    task_mix = [
        ...
        *[
            (("create_simple", "create a new file", 2, 0.1, 10, "code_generation"))
            for _ in range(25)
        ],
    ...
    print(
        f"  Planning Gate: {gate_stats['fast_path_count']} fast-paths / {gate_stats['total_decisions']} decisions"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Validate arXiv optimization targets")
    parser.add_argument(
        "--tasks",
        type=int,
        default=100,
