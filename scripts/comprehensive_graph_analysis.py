...
def analyze_connected_components(G: nx.DiGraph) -> Dict:
    ...
    return {
        ...
        "largest_weak_component_ratio": (
            len(weak_components[0]) / G.number_of_nodes() if weak_components else 0
        ),
    }
...
def analyze_dead_code(G: nx.DiGraph, nodes: Dict) -> Dict:
    ...
    for node_id, node in nodes.items():
        if node["type"] in ("function", "class"):
            ...
            meaningful_in_edges = [
                e for e in in_edges if e[2].get("edge_type") not in ("CONTAINS",)
            ]
        ...
        coupling_analysis.append(
            {
                ...
                "instability": (
                    round(out_coupling / (in_coupling + out_coupling), 3)
                    if (in_coupling + out_coupling) > 0
                    else 0
                ),
            }
        )
...
def analyze_graph_metrics(G: nx.DiGraph) -> Dict:
    ...
    for node_id in G.nodes():
        ...
        module_dist[mod] += 1
    metrics["module_distribution"] = dict(
        sorted(module_dist.items(), key=lambda x: x[1], reverse=True)
    )
...
def generate_module_dependency_data(G: nx.DiGraph, nodes: Dict) -> Dict:
    ...
    for (src, dst), edge_types in module_edges.items():
        ...
        if total >= 5:  # Only significant edges
            dependencies.append(
                {"source": src, "target": dst, "total": total, "by_type": dict(edge_types)}
            )
...
def generate_summary_report(results: Dict):
    ...
    with open(report_file, "w") as f:
        ...
        for typ, count in sorted(
            metrics["node_type_distribution"].items(), key=lambda x: x[1], reverse=True
        ):
            f.write(f"| {typ} | {count:,} |\n")
        ...
        for i, node in enumerate(results["centrality"]["pagerank"][:20], 1):
            f.write(
                f"| {i} | {node['name'][:30]} | {node['type']} | {node['module']} | {node['score']:.6f} |\n"
            )
        ...
        for m in coupling["module_coupling"][:15]:
            f.write(
                f"| {m['module']} | {m['afferent_coupling']} | {m['efferent_coupling']} | {m['total_coupling']} | {m['instability']:.2f} |\n"
            )
