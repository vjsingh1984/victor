...
for i, comp in enumerate(weak_comps[:5]):
    ...
    comp_details.append(
        {"id": i + 1, "size": len(comp), "modules": dict(modules), "types": dict(types)}
    )
    ...
    json.dump(
        {
            ...
            "by_severity": {
                "medium": len([x for x in dead_code if x["severity"] == "medium"]),
...
for c in coupling[:15]:
    print(
        f"  {c['module']:27} | {c['afferent']:6} | {c['efferent']:6} | {c['total']:6} | {c['instability']:.2f}"
    )
        ...
        pattern_dups.append(
            {
                ...
                "functions": [
                    {
                        "name": victor_nodes[n]["name"],
                        "module": get_module(victor_nodes[n]["file"]),
                    }
                    for n in nids[:5]
                ],
...
for (src, dst), count in sorted(
    [
        ...
        for s in module_edges
        for d in module_edges[s]
    ],
    key=lambda x: module_edges[x[0][0]][x[0][1]],
    reverse=True,
):
