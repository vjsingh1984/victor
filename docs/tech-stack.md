...
| TD-21 | Usage attribution wiring | Adopt the `sandhi` OSS usage gateway (own repo `anvai-labs/sandhi`) for per-user/per-team attribution + shared-key virtual keys. The load-bearing gap: `_verify_api_key` in `victor/integrations/api/fastapi_server.py` already resolves `Authorization: Bearer` → `client_id`, but that identity is **never joined to the cost records** (`SessionCostTracker`/`UsageAnalytics` stay session-scoped). Phase 1 = carry `subject_id`/`group_id` into cost records (Victor-local, non-breaking, default `None`); Phase 2 = `sandhi` middleware over `BaseProvider` + local sink + per-subject display; Phase 3 = reverse-proxy serve mode (virtual keys, budgets, rate limits) for internal-network shared-key teams; Phase 4 = migrate Victor's ~28 hand-rolled Python provider adapters onto the Rust `sandhi-providers` transport (ADR-0047 D10) provider-by-provider behind a flag (OpenAI-compat first → Anthropic → rest), Python adapters as fallback + a Python escape hatch for custom providers, Victor keeping prompt assembly / FEP-0011 hints / tool translation. Decision docs: FEP-0020, ADR-018, AnvaiOps ADR-0047. Victor stays on the OSS side of the open-core line (attribution/budgets OSS; billing/governance are AnvaiOps). Status 2026-07-20: Phases 1–3 shipped (#567/#568) + Phase-4a proxy routing (#569); Sandhi's async PyO3 transport binding shipped, so the remaining work = route Victor's own usage-parsing through `parse_usage` (D10a step 2), auto-attach the meter (`usage_gateway.enabled`), and the flag-gated in-process transport pilot (Phase 4b, default OFF). | High | In progress | `victor/integrations/`, `victor/agent/`, `victor/providers/` |
