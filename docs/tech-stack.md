...
| TD-20 | Framework stdout log volume | An agent run logs every turn, provider round-trip, and per-workspace DB migration at INFO/WARNING (`victor.agent`, `victor.providers`, `victor.core.database`, `victor.evaluation.agent_adapter`, `victor.framework.agentic_loop`). Redirected to a file this is unbounded — a stuck real-agent calibration wrote **~350 GB** of trajectories to one log and filled the disk (2026-07-06; held open by the live PID so `rm` freed nothing until killed). Mitigated at the tooling layer (`configure_logging` in the calibration runner, quiet by default, #428) but the framework itself should not emit full trajectories at INFO or duplicate idempotent migration warnings per workspace. Fix: demote per-turn/per-migration logs to DEBUG (or truncate trajectory content), and make migration-idempotency a single logged summary, not one line per DB. | Medium | Planned | `victor/framework/`, `victor/core/database.py` |
