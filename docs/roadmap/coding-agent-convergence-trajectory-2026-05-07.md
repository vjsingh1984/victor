...
| --- | --- | --- | --- | --- |
| Critical | 4 | Reproducible published benchmark suites (`issue-fix`, `review bug-catch`, merge cost/time) | In progress | Task reports, merge-efficiency metrics, reusable report bundles, multi-artifact compare, runtime-variant summaries, fixture manifests, bundled local result copies, checksummed bundle artifacts, manifest-loadable fixture bundles, checked-in fixture-set examples, fixture-set discovery, compare-by-name fixture-set loading, alias-aware benchmark-scoped fixture loading, fixture verification, benchmark-level fixture corpus discovery, exportable metadata-rich fixture corpus catalogs, portable fixture benchmark publication bundles, direct publication-catalog consumption in the shared compare/load path, stable benchmark-model and publisher summary output, and explicit full catalog-coverage reporting are live across checked-in `swe-bench`, `swe-bench-lite`, `humaneval`, `mbpp`, `mbpp-test`, `dr3-eval`, `clawbench`, `guide`, and `vlaa-gui` examples; the remaining gap is durable benchmark corpora depth and broader public stable-run fixture publication. |
| Critical | 3 | Practical default worktree-isolated delegation flow | In progress | Worker contracts, merge-review contracts, auto-materialized delegate worktrees, safe auto-merge for execution-eligible plans, preserved follow-up worktrees, executable delegate re-entry contracts, targeted retry briefs, explicit delegate approval contracts, approval-carried resume payloads plus member-level retry briefs, actionable delegate `next_steps` contracts for review/retry/merge follow-up, runnable `delegate_merge_contract` approval resumes, direct `delegate_next_step` execution, compact `delegate_next_step_id` resume/merge selection from approval or follow-up contracts, top-level surface-ready follow-up step catalogs with `primary_step_id`, directly executable task-plus-context `primary_step_execution_request` / `step_execution_requests` payloads, direct coordinator execution of those follow-up request envelopes, direct coordinator execution from the full follow-up contract with compatibility fallback for older step-only contract shapes, the optional `IDelegateFollowUpCoordinator` protocol capability, workflow `TeamStep` routing for follow-up contracts through that capability, and a `victor workflow run --delegate-follow-up-contract ... --delegate-next-step-id ...` user-facing resume entry point are live; the remaining gap is fuller delegated review/fix retry ergonomics across chat/TUI surfaces. |
| Critical | 2 | Code-intelligence-first live navigation | In progress | The runtime now blocks or rewrites broad code reads toward `lsp`, `symbol`, `refs`, and `project_overview`, including symbol-aware empty-result fallback and follow-up narrowing from recent `refs`/`project_overview` results into targeted reads or symbol lookup; the remaining gap is broader multi-step follow-through and tighter default UX documentation. |
| High | 1 | Remove remaining deprecated runtime wrapper surfaces | Done | Deprecated chat runtime wrapper methods and the module-level `ChatCompatRuntimeProtocol` surface are removed; canonical helper getters are now the only supported planning/context-limit runtime seam. |
