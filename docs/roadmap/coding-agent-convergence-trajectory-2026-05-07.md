...
| --- | --- | --- | --- | --- |
| Critical | 4 | Reproducible published benchmark suites (`issue-fix`, `review bug-catch`, merge cost/time) | In progress | Task reports, merge-efficiency metrics, reusable report bundles, multi-artifact compare, and runtime-variant summaries are live; the remaining gap is durable benchmark corpora plus checked-in/public result fixtures. |
| Critical | 3 | Practical default worktree-isolated delegation flow | In progress | Worker contracts, merge-review contracts, auto-materialized delegate worktrees, and safe auto-merge for execution-eligible plans are live; the remaining gap is turnkey review/fix loops and stricter default cleanup/approval UX. |
...
| High | 4 | Distinguish runtime-policy and tool-selection effects in reports | In progress | Planning-policy deltas, code-intelligence tool-selection deltas, and prompt/runtime variant metadata are now preserved in comparison bundles; the remaining gap is richer public breakdowns on stable corpora. |
