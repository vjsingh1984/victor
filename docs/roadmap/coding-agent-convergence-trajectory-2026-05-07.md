...
| Critical | 3 | Practical default worktree-isolated delegation flow | In progress | Worker contracts, merge-review contracts, auto-materialized delegate worktrees, safe auto-merge for execution-eligible plans, preserved follow-up worktrees, executable delegate re-entry contracts, targeted retry briefs, and explicit delegate approval contracts are live; the remaining gap is fuller delegated review/fix retry ergonomics and surface-level approval UX on top of that resume path. |
