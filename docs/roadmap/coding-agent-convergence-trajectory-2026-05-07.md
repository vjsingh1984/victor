...
| Critical | 3 | Practical default worktree-isolated delegation flow | In progress | Worker contracts, merge-review contracts, auto-materialized delegate worktrees, safe auto-merge for execution-eligible plans, preserved follow-up worktrees, executable delegate re-entry contracts, targeted retry briefs, explicit delegate approval contracts, and approval-carried resume payloads plus member-level retry briefs are live; the remaining gap is fuller delegated review/fix retry ergonomics across surface layers. |
