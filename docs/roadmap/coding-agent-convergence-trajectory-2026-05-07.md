...
| --- | --- | --- | --- | --- |
| Critical | 3 | Workspace-isolated delegation as the canonical parallel-work seam | In progress | `WorkspaceIsolationService` owns planning, materialization, changed-file collection, merge orchestration, and cleanup policy; `WorkspaceIsolationPolicy` centralizes mode/materialization/dry-run/merge/cleanup flags; `GitWorktreeRuntime` now emits structured stale-path, existing-branch, missing-member, integration-artifact, cleanup, and blocked-reason diagnostics while public `worktree_*` payloads are preserved. Next: surface these diagnostics in task reports and user-facing follow-up contracts. |
| Critical | 3 | Deterministic delegate follow-up and resume contracts | In progress | Worker contracts, merge-review contracts, preserved follow-up worktrees, direct follow-up request/contract execution, `IDelegateFollowUpCoordinator`, workflow `TeamStep` routing, and `victor workflow run --delegate-follow-up-contract ... --delegate-next-step-id ...` are live. Next: make chat/TUI follow-up resume ergonomics consume the same workflow/team capability without adding local delegate runtimes. |
...
- Phase 4: benchmark fixture catalogs, publication bundles, runtime-variant summaries, and merge-efficiency metrics exist; public stable-run depth remains the gap.

## Refined Next Execution Order

| Order | Priority | Phase | Work Item | Correct Seam | Done When |
| --- | --- | --- | --- | --- | --- |
| 1 | Critical | 3 | Surface workspace diagnostics in reports and follow-up contracts | `WorkspaceIsolationService` + `MetricsCoordinator` + delegate contract builders | Stale paths, existing branches, integration worktree leftovers, cleanup failures, and blocked merge reasons are visible in task reports and actionable follow-up contracts. |
| 2 | Critical | 3 | Productize delegate follow-up ergonomics across surfaces | Workflow/`TeamStep` capability first; chat/TUI only compose it | Users can resume review/retry/merge/cleanup contracts from chat/TUI with concise return contracts and no local duplicate delegate runtime. |
| 3 | Critical | 4 | Publish stable benchmark corpora and run outputs | Benchmark fixture/catalog/report pipeline | Public reports include issue-fix success, review bug-catch, tokens-to-merge, time-to-first-edit, and cost per accepted patch across stable corpora. |
| 4 | Critical | 2 | Complete code-intelligence-first default navigation | `ToolService`/tool pipeline/LSP tools | Default coding workflow completes common navigation through symbols/diagnostics/refs/project overview before broad reads, with regression tests. |
| 5 | High | 4 | Report workspace-policy effects | `MetricsCoordinator` and benchmark report exporters | Reports distinguish model, runtime policy, workspace policy, and tool-selection effects. |
| 6 | High | 2 | Narrow and document default UX | mode controller, slash/CLI docs, coding-agent docs | `plan`, `build`, `review`, and `delegate` are the documented default surface; advanced framework controls are explicit opt-in. |
| 7 | High | 1 | Continue `AgentOrchestrator` shrinkage | service owners only | New production behavior is owned by `ChatService`, `ToolService`, `ContextService`, `ProviderService`, `SessionService`, `RecoveryService`, or team workspace services. |
| 8 | Medium | 0-1 | Preserve completed guardrails | focused regression suites | Instruction discovery, task reports, compaction continuity, and model-aware continuation stay green while later phases move. |
