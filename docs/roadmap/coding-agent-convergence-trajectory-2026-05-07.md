...
| Critical | 3 | Practical default worktree-isolated delegation flow | In progress | Worker contracts, merge-review contracts, auto-materialized delegate worktrees, safe auto-merge for execution-eligible plans, preserved follow-up worktrees, executable delegate re-entry contracts, and targeted retry briefs are live; the remaining gap is tighter approval UX and fuller delegated review/fix retry ergonomics on top of that resume path. |
