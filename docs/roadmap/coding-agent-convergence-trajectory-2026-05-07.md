...
| Critical | 3 | Practical default worktree-isolated delegation flow | In progress | Worker contracts, merge-review contracts, auto-materialized delegate worktrees, safe auto-merge for execution-eligible plans, and preserved follow-up worktrees with fix/review queues are live; the remaining gap is tighter approval UX and a fuller delegated re-entry/retry loop. |
