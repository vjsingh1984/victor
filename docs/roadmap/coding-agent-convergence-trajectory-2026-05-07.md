...
- Phase 2: `tool_pipeline.py` steers broad code reads in `plan`, `build`, `review`, `delegate`, and `explore` toward `lsp`, `symbol`, `refs`, and `project_overview`; the default `victor chat --mode` surface is now narrowed to `build`, `plan`, `review`, and `delegate`, with `explore` and workflow controls explicit opt-ins.
