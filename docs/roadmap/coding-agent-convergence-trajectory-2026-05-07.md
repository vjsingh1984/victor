...
| Critical | 4 | Reproducible published benchmark suites (`issue-fix`, `review bug-catch`, merge cost/time) | In progress | Task reports, merge-efficiency metrics, reusable report bundles, multi-artifact compare, fixture manifests, portable fixture publication bundles, real-run stable publication bundles, publication-catalog consumption, stable publisher summaries, full catalog-coverage reporting, and per-corpus `stable_run_summary.json` KPI sidecars are live. Next: populate durable non-fixture corpora from actual benchmark executions and publish those checked artifacts. |
...
- Phase 4: benchmark fixture catalogs, fixture and real-run publication bundles, stable-run KPI sidecars, runtime-variant summaries, merge-efficiency metrics, workspace-policy/diagnostic rollups, and workspace-policy comparison fields exist; public real-run corpus depth remains the gap.
