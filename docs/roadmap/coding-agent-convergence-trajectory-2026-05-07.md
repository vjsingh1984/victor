...
| --- | --- | --- | --- | --- | --- |
| 1 | Critical | 3 | Productize selectable delegate follow-up summaries | Workflow/`TeamStep` capability first; TUI only selects/summarizes contracts | Users can pick review/retry/merge/cleanup follow-up steps from concise saved-contract summaries and execute them through `/delegate-follow-up`, with no local duplicate delegate runtime. |
