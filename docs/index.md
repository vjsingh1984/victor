<div align="center">

# Victor AI Documentation Index

**Navigation hub for the Victor documentation set**

[![Version](https://badge.fury.io/py/victor-ai.svg)](https://pypi.org/project/victor-ai/)
[![Documentation](https://img.shields.io/badge/docs-latest-blue.svg)](./index.md)

</div>

---

## 🎯 Choose Your Journey

New to Victor? Choose a guided learning path tailored to your role and goals.

| Journey | For | Time | Outcome |
|---------|-----|------|---------|
| [🚀 Beginner](./journeys/beginner.md) | New users | 30 min | Ready for daily use |
| [💡 Intermediate](./journeys/intermediate.md) | Daily users | 80 min | Power user |
| [👨‍💻 Developer](./journeys/developer.md) | Contributors | 2 hours | Active contributor |
| [🔧 Operations](./journeys/operations.md) | DevOps/SRE | 80 min | Production deployment |
| [🏗️ Advanced](./journeys/advanced.md) | Architects | 2.5 hours | System expert |

→ [View All Journeys](./journeys/index.md)

---

## Quick Start

| Document | Description | Reading Time |
|----------|-------------|--------------|
| [Getting Started](./getting-started/index.md) | Entry point for setup and first run | 10 min |
| [Installation](./getting-started/installation.md) | Install via pip, pipx, or Docker | 5 min |
| [First Run](./getting-started/first-run.md) | Initialize your environment | 3 min |
| [Configuration](./getting-started/configuration.md) | Profiles and settings | 10 min |
| [User Guide](./user-guide/index.md) | Day-to-day usage | 30 min |
| [FAQ](./user-guide/faq.md) | Frequently asked questions | 5 min |
| [Troubleshooting](./getting-started/troubleshooting.md) | Common issues and fixes | 10 min |

---

## Welcome

Victor AI is an open-source AI coding assistant with multiple providers, tools, and domain verticals. This index points to the canonical docs and avoids version-specific counts that change over time.

---

## Contributors

| Document | Description |
|----------|-------------|
| [Contributing Index](./contributing/index.md) | Canonical contributor hub |
| [Setup](./contributing/setup.md) | Dev environment setup |
| [Code Style](./contributing/code-style.md) | Formatting and linting |
| [Testing](./contributing/testing.md) | Test strategy and commands |
| [Documentation Guide](./contributing/documentation-guide.md) | Doc workflow and structure |
| [CI/CD](./ci_cd/README.md) | CI/CD docs |
| [Testing Hub](./testing/README.md) | Test suites and QA |
| [Root CONTRIBUTING](contributing/index.md) | Repo-level contribution guide |

---

## Core Documentation

### Architecture & Design

- [Architecture Overview](./architecture/overview.md)
- [Architecture Index](./architecture/INDEX.md)
- [Component Reference](./architecture/COMPONENT_REFERENCE.md)
- [Protocols Reference](./architecture/PROTOCOLS_REFERENCE.md)
- [Design Patterns](./architecture/DESIGN_PATTERNS.md)
- [Diagrams](./diagrams/index.md)

### Reference

- [Reference Index](./reference/index.md)
- [API Reference](./reference/api/index.md)
- [Configuration Reference](./reference/configuration/index.md)
- [Providers](./reference/providers/index.md)
- [Tools](./reference/tools/catalog.md)
- [Errors](./reference/errors.md)

### Tutorials & Guides

- [Tutorials Index](./tutorials/README.md)
- [Guides Index](./guides/index.md)
- [Workflow Quickstart](./guides/workflow-quickstart.md)
- [Vertical Quickstart](./guides/vertical-quickstart.md)

### Verticals

- [Verticals Overview](./reference/verticals/index.md)
- [Coding](./verticals/coding.md)
- [DevOps](./verticals/devops.md)
- [RAG](./verticals/rag.md)
- [Data Analysis](./verticals/data-analysis.md)
- [Research](./verticals/research.md)

---

## Operations & Production

- [Operations Index](./operations/index.md)
- [Production Runbooks](./production/README.md)
- [Observability](./observability/README.md)
- [Security](./security/scanning.md)

---

## Performance, Workflows, Extensions

- [Performance Hub](./performance/README.md)
- [Workflows Overview](./workflows/COMMON_WORKFLOWS.md)
- [Extensions](./extensions/README.md)
- [Caching](./caching/README.md)

---

## Reports & Historical Material

Historical reports and internal snapshots live alongside the docs. Treat metrics and completion claims as time-bound unless explicitly tied to a release or external audit.

- [Changes](changes/phase_1_1_proximadb_optional.md)
- [Refactoring](refactoring/phase2_property_accessor_reduction.md)
- [Internal API Docs](./reference/internals/INDEX.md)
- [Roadmap](./roadmap/future_roadmap.md)
- [Success Stories](./stories/SUCCESS_STORIES.md)
- [User Stories](./stories/user_stories.md)

---

<div align="center">

**[<- Back to Top](#victor-ai-documentation-index)**

**Victor AI Documentation**

</div>
