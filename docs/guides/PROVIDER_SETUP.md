...
![Victor Provider Ecosystem](../assets/provider-comparison.svg)
...
![Provider Setup in 3 Steps](../assets/provider-quickstart.svg)
...
```mermaid
flowchart TD
    Start[Error Occurred] --> E1{Error Type?}

    E1 -->|Provider not found| P1[Check provider registered]
    P1 --> P1a["victor providers"]

    E1 -->|401 Unauthorized| A1[Check API key]
    A1 --> A1a["victor keys --list"]
    A1a --> A1b[Verify key not expired]

    E1 -->|429 Rate Limited| R1[Wait and retry]
    R1 --> R1a[Switch provider]

    E1 -->|503 Model Loading| M1[Cold start - retry in 30s]

    E1 -->|Connection Refused| C1[Check local server]
    C1 --> C1a["ollama serve / LM Studio"]

    style Start fill:#f87171,stroke:#dc2626
    style P1a fill:#86efac,stroke:#22c55e
    style A1a fill:#86efac,stroke:#22c55e
    style R1a fill:#fde047,stroke:#eab308
```
