...
async def analyze_csv(file_path: str):
    ...
    result = await agent.run(f"""Analyze the CSV file at {file_path}.
...
async def generate_plot_code(data_description: str, plot_type: str = "bar"):
    ...
    agent = Agent.create(tools=["python"], vertical="dataanalysis", temperature=0.4)
...
async def data_cleaning_suggestions(data_description: str):
    ...
    agent = Agent.create(vertical="dataanalysis", temperature=0.4)
    result = await agent.run(f"""Suggest data cleaning steps for:
        {data_description}
...
async def statistical_analysis(file_path: str):
    ...
    result = await agent.run(f"""Perform statistical analysis on data in {file_path}.
...
async def main():
    ...
    plot_code = await generate_plot_code("Monthly sales data for 12 months", "line")
    print(plot_code[:300] + "...\n")
