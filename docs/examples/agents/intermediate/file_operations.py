...
async def search_codebase(query: str, path: str = "."):
    ...
    agent = Agent.create(tools=["grep", "read"], temperature=0.2)

    result = await agent.run(f"""Search the codebase in {path} for: {query}
...
async def analyze_file_structure(directory: str):
    ...
    agent = Agent.create(tools=["ls", "read"], temperature=0.3)

    result = await agent.run(f"""Analyze the file structure in {directory}.
...
async def find_unused_imports(file_path: str):
    ...
    agent = Agent.create(tools=["read", "grep"], vertical="coding", temperature=0.2)

    result = await agent.run(f"""Analyze {file_path} for unused imports.
...
async def generate_file_documentation(file_path: str):
    ...
    agent = Agent.create(tools=["read"], vertical="coding", temperature=0.4)

    result = await agent.run(f"""Generate comprehensive documentation for {file_path}.
