...
async def review_commit(message: str = None):
    ...
    agent = Agent.create(tools=["git_status", "git_diff"], temperature=0.3)
    ...
    if not message:
        ...
        result = await agent.run(f"""Review this commit message: "{message}"
...
async def generate_commit_description():
    ...
    agent = Agent.create(tools=["git_status", "git_diff", "read"], temperature=0.4)
...
async def analyze_git_history():
    ...
    agent = Agent.create(tools=["git"], temperature=0.3)

    result = await agent.run("""Analyze the last 10 commits.
...
async def create_branch_suggestion(issue_description: str):
    ...
    result = await agent.run(f"""Based on this issue: "{issue_description}"
