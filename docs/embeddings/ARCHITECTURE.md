...
flowchart TB
    MSG["💬 User Message"]
    SEL["SemanticToolSelector.select()"]

    MSG --> SEL

    SEL -->|Default: ON| SEM["Semantic Selection"]
    SEL -->|if disabled| KEY["Keyword Fallback"]

    SEM --> EMB["_get_embedding(user_message)"]

    subgraph PROVIDERS["Embedding Providers"]
        direction LR
        ST["**sentence-transformers**<br/>80MB • ~5ms<br/>✅ OFFLINE • DEFAULT"]
        OL["**Ollama API**<br/>~5GB • ~50-100ms<br/>ONLINE • OPTIONAL"]
        VL["**vLLM API**<br/>~5GB • ~50-100ms<br/>ONLINE • OPTIONAL"]
        LM["**LMStudio API**<br/>~5GB • ~50-100ms<br/>ONLINE • OPTIONAL"]
    end

    EMB --> ST & OL & VL & LM

    style MSG fill:#e0e7ff,stroke:#4f46e5
    style SEL fill:#d1fae5,stroke:#10b981
    style SEM fill:#fef3c7,stroke:#f59e0b
    style KEY fill:#fee2e2,stroke:#ef4444
    style ST fill:#d1fae5,stroke:#10b981
    style OL fill:#f0f9ff,stroke:#0284c7
    style VL fill:#f0f9ff,stroke:#0284c7
    style LM fill:#f0f9ff,stroke:#0284c7
```
