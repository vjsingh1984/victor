...
- **F-016f · Credit-assignment *feedback* loop — DONE (PR #504), verdict was wire.** Re-verified: `assign_turn_credit()` (`framework/rl/credit_tracking_service.py:260`) had zero production callers (docstring + tests only); `auto_assign_at_turn_boundary: bool = True` (`config/credit_assignment_settings.py:23`) was never read anywhere; `_turn_count` advances only inside `assign_turn_credit` (:288), so both the `get_tool_credit_summary` (empty) and the `_turn_count < 3` guard (:490) kept `generate_tool_guidance()` returning `None` despite live wiring at `prompt_pipeline.py:905`. **Wired** at the turn boundary in `agent/services/turn_execution_runtime.py`: after `transition_coordinator.end_turn()` in `execute_turn`, a gated `_maybe_assign_turn_credit(_orch)` drains the turn's accumulated tool signals — no-op unless credit tracking is enabled (service is `None` otherwise) and `auto_assign_at_turn_boundary` is set (the flag's first reader). Per-signal agent/team/session ids captured at record time take precedence, so the default call is correct; toolless turns are a cheap no-op. Sibling of F-002. Tests: 5 gating tests on the wire (`TestMaybeAssignTurnCredit`); the guard-clearing end-to-end is already covered by `test_guidance_generated_after_sufficient_data`.
- **F-016g · Post-switch hooks — STILL VALID, verdict moved wire → DELETE.** New provenance (re-verify 2026-07-12): the dispatching `ProviderSwitchCoordinator` was **already deleted in `d50ec336` (v0.7.0)** — `post_switch_hooks.py` (316 LOC; its comments still reference the removed `AgentOrchestrator._apply_post_switch_hooks()`) is an orphaned leftover of a deleted subsystem, not an unwired feature. The wire seam already exists elsewhere: `ProviderService.register_post_switch_hook()` (:596) / `_notify_post_switch_hooks()` (:615) — zero registrants — and the live switch path already re-inits tool adapter + capabilities + model switcher (`provider_manager.py:446-459`). **Action**: delete the module + its `provider/__init__.py` exports; if a mid-session switch ever needs prompt/system-prompt/tool-budget refresh, register callbacks on the existing `ProviderService` seam rather than resurrecting the hook classes.
