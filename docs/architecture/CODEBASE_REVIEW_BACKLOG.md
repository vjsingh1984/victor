...
- **F-016m · `tool_calling_caps` is never re-derived on a mid-session provider/model switch — STALE-STATE bug.** Verified: `tool_calling_caps` is a construction-time field on `OrchestratorComponents` (`orchestrator_factory.py:165`), built once from the *initial* model; **nothing in the switch path re-derives it** (`provider_management_runtime.py::switch_provider`/`switch_model` → `_sync_runtime_provider_state` updates only `provider`/`provider_name`/`model`; `provider_service.switch_*` does not touch caps). So after a mid-session switch, `orchestrator.tool_calling_caps` reflects the OLD model. This is why F-016g's "exploration-refresh" residual can't be a clean 3-liner: reading `tool_calling_caps.exploration_multiplier` post-switch yields the stale (old-model) value, so the refresh would be a no-op. The staleness is broader than exploration — the same never-refreshed caps back **`native_tool_calls`** (`orchestrator.py:487`), **`tool_budget`** (`:1031` via `initialize_tool_budget`), and **`thinking_disable_prefix`** (`:3074`). Impact is bounded (heuristic tuning + switch is not the hot path), so **deferred by decision (2026-07-12)** rather than hacked. Proper fix (own scoped PR if pursued): re-derive `tool_calling_caps` for the new model in the switch path (via `ProviderService` capability discovery) and refresh the derived state (exploration on the tracker + tool_budget); do NOT ship an exploration-only refresh that reads stale caps. Supersedes the "Residual" note under F-016g.
