...
- Planning runtime compaction now follows the same service-first path before using the legacy context compactor fallback.
- The legacy async `ContextManager` compaction path now accepts a context service and uses it before the direct compactor fallback.
