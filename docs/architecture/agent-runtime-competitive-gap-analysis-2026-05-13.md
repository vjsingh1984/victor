...
- StateGraph checkpoint saving now binds the generated `WorkflowCheckpoint.checkpoint_id` into execution checkpoint context before persistence and before graph snapshots/stream callbacks, linking graph state checkpoints to the unified envelope.
