...
class TestGitTool:
    ...
    async def test_git_commit(self):
        ...
        with patch("victor.tools.git_tool._run_git_async", new_callable=AsyncMock) as mock:
            ...
            assert result["success"] is True
            commit_message = mock.call_args.args[2]
            assert "Generated-by: victor-code-ai" in commit_message
            assert (
                "Co-authored-by: victor-code-ai "
                "<296282796+victor-code-ai@users.noreply.github.com>"
            ) in commit_message
            assert "attribution trailers were recorded" in result["output"]
    ...
    async def test_pr_creation_success(self):
        ...
        with patch("victor.tools.git_tool._run_git_async", new_callable=AsyncMock) as mock_git:
            ...
            with patch(
                ...
                assert "Generated by victor-code-ai." in command
                assert "PR body includes Victor AI attribution" in result["output"]
