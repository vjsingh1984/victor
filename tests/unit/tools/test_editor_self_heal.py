...
from victor_coding.editing.editor import EditOperation, FileEditor, OperationType  # noqa: E402


def _commit_modify(editor: FileEditor, path: str, content: str) -> bool:
...
def test_edit_self_heals_after_backup_dir_deleted(tmp_path):
    ...
    assert _commit_modify(editor, "x.py", "a = 2\n") is True
    ...
    assert _commit_modify(editor, "x.py", "a = 3\n") is True
