...
def test_verify_vertical_class_reports_contract_base_namespace(tmp_path: Path) -> None:
    (tmp_path / "victor-vertical.toml").write_text(
        """
[vertical.class]
module = "bad_vertical"
class_name = "BadVertical"
    ...
    with pytest.raises(TypeError, match="victor_contracts\\.VerticalBase"):
        verify_vertical_class(tmp_path)
