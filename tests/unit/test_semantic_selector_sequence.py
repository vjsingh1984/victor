...
class TestRecordToolUsage:
    ...
    def test_record_tool_usage_updates_sequence_tracker(self):
        """Test that recording tool usage updates the sequence tracker."""
        with tempfile.TemporaryDirectory() as tmpdir:
            selector = SemanticToolSelector(cache_dir=Path(tmpdir))
    ...
    def test_record_tool_usage_records_success(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            selector._record_tool_usage("read_file", "query", success=True)
            selector._record_tool_usage("edit_files", "query", success=False)
...
class TestGetSequenceBoost:
    ...
    def test_sequence_boost_after_read_file(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            selector._record_tool_usage("read_file", "test", success=True)
...
class TestGetCurrentWorkflow:
    ...
    def test_detect_file_editing_workflow(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            selector._record_tool_usage("read_file", "test", success=True)
            selector._record_tool_usage("edit_files", "test", success=True)
            selector._record_tool_usage("run_tests", "test", success=True)
