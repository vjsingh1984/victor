...
class TestSessionsCommand:
    ...
    def test_sessions_list_json_includes_preview_count(
        self, runner_with_db, temp_db_path
    ):
        ...
        preview_session = next(
            ...
            if session["session_id"] == "myproj-preview-list"
        )
    ...
    def test_sessions_show_json_preserves_preview_messages(
        self, runner_with_db, temp_db_path
    ):
        ...
        result = runner_with_db.invoke(
            sessions_app, ["show", "myproj-preview01", "--json"]
        )
        assert result.exit_code == 0
    ...
    def test_sessions_show_displays_preview_messages(
        self, runner_with_db, temp_db_path
    ):
