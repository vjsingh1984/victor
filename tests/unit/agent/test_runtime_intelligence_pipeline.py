...
class TestProcessResponse:
    ...
    @pytest.mark.asyncio
    async def test_process_response_keeps_retry_for_code_task_reference_issues(
        self, pipeline
    ):
    ...
    async def test_process_response_builds_grounding_evidence_summary(self, pipeline):
        ...
        assert (
            "- Verified: src/main.rs, src/main.rs:10"
            in result.grounding_evidence_summary
        )
...
class TestModuleFunctions:
    ...
    @pytest.mark.asyncio
    async def test_get_runtime_intelligence_pipeline_different_keys(
        self, mock_provider_adapter
    ):
    ...
    def test_clear_runtime_intelligence_pipeline_cache(self, mock_provider_adapter):
        ...
        _runtime_intelligence_pipeline_cache[("test", "test", "test", None)] = (
            test_pipeline
        )
    ...
    def test_get_optimal_tool_budget_delegates_to_mode_controller(
        self, mock_provider_adapter
    ):
        ...
        with patch.object(
            pipeline, "_get_mode_controller", return_value=mock_controller
        ):
