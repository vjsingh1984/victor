...
def test_record_prompt_candidate_outcome_emits_one_event_per_identity(monkeypatch):
    """record_prompt_candidate_outcome emits a PROMPT_CANDIDATE_USED event per
    served identity carrying the turn outcome (FEP-0017 attribution)."""
    from victor.framework.rl.hooks import RLEventType

    service = RuntimeIntelligenceService(optimization_injector=None)
    service._last_served_prompt_identities = [
        PromptOptimizationIdentity(
            provider="zai",
    ...
    service.record_prompt_candidate_outcome(
    ...
    for ev in captured:
        ...
        assert ev.metadata["prompt_candidate_hash"] in {"h1", "h2"}
        emitted_hashes.add(ev.metadata["prompt_candidate_hash"])
    assert emitted_hashes == {"h1", "h2"}


def test_record_prompt_candidate_outcome_noop_without_identities():
    """No identities served this turn → no emission, no raise."""
    service = RuntimeIntelligenceService(optimization_injector=None)
    service._last_served_prompt_identities = []
    # Must not raise even though no hooks are registered.
    service.record_prompt_candidate_outcome(0.5, True)
