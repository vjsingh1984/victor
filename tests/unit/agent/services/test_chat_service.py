...
class TestChatServiceAdapter:
    """Tests for ChatServiceAdapter (the primary chat execution path).

    The adapter wraps ChatCoordinator to preserve the full orchestration
    pipeline (StreamingChatPipeline, AgenticLoop, perception, fulfillment).
    """

    @pytest.mark.asyncio
    async def test_adapter_delegates_chat(self):
        """ChatServiceAdapter.chat() delegates to ChatCoordinator.chat()."""
        from unittest.mock import AsyncMock, MagicMock
        from victor.agent.services.adapters.chat_adapter import ChatServiceAdapter

        mock_coordinator = MagicMock()
        mock_coordinator.chat = AsyncMock(return_value=MagicMock(content="response"))
        adapter = ChatServiceAdapter(mock_coordinator)

        result = await adapter.chat("hello")
        mock_coordinator.chat.assert_called_once_with("hello")
        assert result.content == "response"

    @pytest.mark.asyncio
    async def test_adapter_delegates_stream_chat(self):
        """ChatServiceAdapter.stream_chat() delegates to ChatCoordinator.stream_chat()."""
        from unittest.mock import MagicMock
        from victor.agent.services.adapters.chat_adapter import ChatServiceAdapter

        mock_chunk = MagicMock(content="chunk1")

        async def mock_stream(msg):
            yield mock_chunk

        mock_coordinator = MagicMock()
        mock_coordinator.stream_chat = mock_stream
        adapter = ChatServiceAdapter(mock_coordinator)

        chunks = []
        async for chunk in adapter.stream_chat("test"):
            chunks.append(chunk)
        assert len(chunks) == 1
        assert chunks[0].content == "chunk1"

    @pytest.mark.asyncio
    async def test_adapter_delegates_chat_with_planning(self):
        """ChatServiceAdapter.chat_with_planning() delegates to coordinator."""
        from unittest.mock import AsyncMock, MagicMock
        from victor.agent.services.adapters.chat_adapter import ChatServiceAdapter

        mock_coordinator = MagicMock()
        mock_coordinator.chat_with_planning = AsyncMock(return_value=MagicMock(content="plan"))
        adapter = ChatServiceAdapter(mock_coordinator)

        result = await adapter.chat_with_planning("plan this")
        mock_coordinator.chat_with_planning.assert_called_once()

    def test_adapter_has_health_check(self):
        """Adapter exposes is_healthy()."""
        from unittest.mock import MagicMock
        from victor.agent.services.adapters.chat_adapter import ChatServiceAdapter
