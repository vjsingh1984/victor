# Copyright 2025 Vijaykumar Singh <singhvjd@gmail.com>
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.

"""Integration parity checks for the RAG vertical migration."""

from __future__ import annotations

import pytest

from victor.config.settings import Settings
from victor.core.bootstrap import bootstrap_container
from victor.core.container import ServiceContainer, set_container
from victor.core.verticals.protocols import VerticalExtensions
from victor.core.verticals.vertical_loader import get_vertical_loader
from victor.framework.vertical_runtime_adapter import VerticalRuntimeAdapter
from victor.verticals.contrib.rag import RAGAssistant as RuntimeRAGAssistant
from victor.verticals.contrib.rag.assistant import RAGAssistant as RAGAssistantDefinition
from victor.verticals.contrib.rag.mode_config import RAGModeConfigProvider
from victor.verticals.contrib.rag.prompts import RAGPromptContributor
from victor.verticals.contrib.rag.rl import RAGRLConfig
from victor.verticals.contrib.rag.safety import RAGSafetyExtension
from victor.verticals.contrib.rag.teams import RAGTeamSpecProvider
from victor.verticals.contrib.rag.workflows import RAGWorkflowProvider
from victor_sdk import CapabilityIds


@pytest.fixture(autouse=True)
def reset_rag_vertical_state() -> None:
    """Reset shared loader/container state between parity checks."""

    loader = get_vertical_loader()
    loader.reset()
    clear_config_cache = getattr(RAGAssistantDefinition, "clear_config_cache", None)
    if callable(clear_config_cache):
        clear_config_cache()
    set_container(ServiceContainer())

    yield

    loader.reset()
    if callable(clear_config_cache):
        clear_config_cache()
    set_container(ServiceContainer())


def test_rag_migration_discovery_parity_preserves_loader_and_binding_contracts() -> None:
    """Discovery should still resolve RAG through the normal loader/runtime path."""

    loader = get_vertical_loader()
    vertical = loader.load("rag")
    binding = VerticalRuntimeAdapter.build_runtime_binding(vertical)
    definition = vertical.get_definition()

    assert vertical.get_name() == RAGAssistantDefinition.get_name()
    assert vertical.__name__ == RAGAssistantDefinition.__name__
    assert loader.active_vertical_name == "rag"
    assert set(loader.get_tools()) == set(RAGAssistantDefinition.get_tools())
    assert loader.get_system_prompt() == definition.system_prompt
    assert set(binding.runtime_config.tools.tools) == set(definition.get_tool_names())
    assert set(binding.runtime_config.stages) == set(definition.stages)
    assert [
        item["capability_id"] for item in binding.runtime_config.metadata["capability_requirements"]
    ] == [
        CapabilityIds.FILE_OPS,
        CapabilityIds.DOCUMENT_INGESTION,
        CapabilityIds.RETRIEVAL,
        CapabilityIds.VECTOR_INDEXING,
        CapabilityIds.WEB_ACCESS,
    ]


def test_rag_migration_activation_parity_registers_runtime_extensions() -> None:
    """Bootstrap activation should still expose RAG runtime integrations."""

    container = bootstrap_container(settings=Settings(), vertical="rag")
    extensions = container.get_optional(VerticalExtensions)

    assert extensions is not None
    assert isinstance(extensions.workflow_provider, RAGWorkflowProvider)
    assert isinstance(extensions.rl_config_provider, RAGRLConfig)
    assert isinstance(extensions.team_spec_provider, RAGTeamSpecProvider)
    assert any(
        isinstance(contributor, RAGPromptContributor)
        for contributor in extensions.prompt_contributors
    )
    assert any(
        isinstance(extension, RAGSafetyExtension) for extension in extensions.safety_extensions
    )
    assert isinstance(extensions.mode_config_provider, RAGModeConfigProvider)
    assert "deep_search" in extensions.get_all_mode_configs()
    assert "question_answering" in extensions.get_all_task_hints()


def test_rag_migration_behavior_parity_uses_shared_runtime_helper_defaults() -> None:
    """Behavior should stay intact after runtime helpers leave the package root."""

    workflow_provider = RuntimeRAGAssistant.get_workflow_provider()
    rl_config = RuntimeRAGAssistant.get_rl_config_provider()
    team_spec_provider = RuntimeRAGAssistant.get_team_spec_provider()

    assert isinstance(workflow_provider, RAGWorkflowProvider)
    assert isinstance(rl_config, RAGRLConfig)
    assert isinstance(team_spec_provider, RAGTeamSpecProvider)
    assert "rag_query" in workflow_provider.get_workflow_names()
    assert "search_team" in team_spec_provider.list_team_types()
    assert "search" in rl_config.task_type_mappings
