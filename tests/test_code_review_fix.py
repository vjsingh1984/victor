...
async def test_code_review_with_single_aspect_string():
    ...
    result = await code_review(
        path="victor/tools", aspects="security", max_issues=5  # Single string
    )
...
async def test_code_review_with_invalid_aspect():
    ...
    result = await code_review(
        path="victor/tools", aspects=["invalid_aspect", "security"], max_issues=5
    )
...
async def test_code_review_all_aspects():
    ...
    result = await code_review(path="victor/tools/base.py", aspects=["all"], max_issues=10)
...
async def test_code_review_report_format():
    ...
    result = await code_review(
        path="victor/tools/base.py", aspects=["security", "best_practices"], max_issues=5
    )
