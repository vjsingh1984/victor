...
class InMemoryEventBackend:
    ...
    def _fire_drop_alert_if_needed(self, topic: str, drop_kind: str) -> None:
        ...
        with self._publish_stats_lock:
            self._per_topic_drop_counts[topic] = (
                self._per_topic_drop_counts.get(topic, 0) + 1
            )
            total = self._publish_stats.get(
                "dropped_newest", 0
            ) + self._publish_stats.get("dropped_oldest", 0)
            delta = total - self._last_alert_total_drops
            if (
                delta >= self._drop_alert_threshold
                and self._drop_alert_callback is not None
            ):
        ...
        try:
            ...
            if effective_policy == _QUEUE_POLICY_DROP_OLDEST:
                ...
                try:
                    ...
                    self._fire_drop_alert_if_needed(
                        event.topic, "drop_newest_after_drop_oldest"
                    )
