...
class ConversationMemoryAdapter:
    ...
    async def search(self, query: MemoryQuery) -> List[MemoryResult]:
        ...
        try:
            ...
            if dual_trace_requested and hasattr(
                self._store, "aget_dual_trace_relevant_messages"
            ):
                dual_trace_results = (
                    await self._store.aget_dual_trace_relevant_messages(
                ...
                return [
                    replace(r, provenance=provenance) for r in results[: query.limit]
                ]
