...
class VerticalIntegrationPipeline:
    ...
    def _apply_middleware(
        ...
        # Use protocol method if available
        if hasattr(orchestrator, "apply_vertical_middleware") and callable(
            orchestrator.apply_vertical_middleware
        ):
            orchestrator.apply_vertical_middleware(middleware_list)
            logger.debug(f"Applied {len(middleware_list)} middleware via protocol")
        else:
    ...
    def _apply_safety(
        ...
        # Use protocol method if available
        if hasattr(orchestrator, "apply_vertical_safety_patterns") and callable(
            orchestrator.apply_vertical_safety_patterns
        ):
            orchestrator.apply_vertical_safety_patterns(all_patterns)
            logger.debug(f"Applied {len(all_patterns)} safety patterns via protocol")
        else:
            # Fallback: try safety checker
    ...
    def _apply_mode_config(
        ...
        if hasattr(orchestrator, "adaptive_mode_controller"):
            controller = orchestrator.adaptive_mode_controller
    ...
    def _apply_tool_deps(
        ...
        if hasattr(orchestrator, "tool_sequence_tracker"):
            tracker = orchestrator.tool_sequence_tracker
    ...
    def _attach_context(
        ...
        """
        # Use protocol method if available
        if hasattr(orchestrator, "set_vertical_context") and callable(
            orchestrator.set_vertical_context
        ):
        ...
        try:
            ...
            if hasattr(vertical, "get_rl_hooks"):
                ...
                if rl_hooks:
                    ...
                    # Attach hooks to orchestrator if it supports them
                    if hasattr(orchestrator, "set_rl_hooks"):
                        orchestrator.set_rl_hooks(rl_hooks)
                    elif hasattr(orchestrator, "_rl_hooks"):
    ...
    def _apply_team_specs(
        ...
        try:
            ...
            # Attach to orchestrator if it supports teams
            if hasattr(orchestrator, "set_team_specs"):
                orchestrator.set_team_specs(team_specs)
            elif hasattr(orchestrator, "_team_specs"):
