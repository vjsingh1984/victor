...
class ToolSelectorLearner(BaseLearner):
    ...
    def _ensure_tables(self) -> None:
        ...
        RLTableMigrator(self.db).run_if_needed(
            self.name, RLTableMigrator.migrate_tool_selector
        )
    ...
    def _load_state(self) -> None:
        ...
        try:
            ...
            for row in cursor.fetchall():
                row_dict = dict(row)
                self._tool_success_counts[row_dict["task_type"]] = int(
                    row_dict["stat_value"]
                )
    ...
    def _save_to_db(
        ...
        cursor.execute(
            ...
            (
                ...
                reward,
                json.dumps(
                    {"success": outcome.success, "quality_score": outcome.quality_score}
                ),
                timestamp,
            ),
        )
