...
from victor.coding.completion.provider import (
    BaseCompletionProvider,
    StreamingCompletionProvider,
)
from victor.coding.completion.registry import (
...
class CompletionManager:
    ...
    async def stream_inline_completion(
        ...
        for provider in self._registry.get_all_providers():
            ...
            try:
                # The method returns AsyncIterator[str], which is awaitable
                ...
                if inspect.iscoroutine(stream_result):
                    ...
                    async_iter: AsyncIterator[str] = await stream_result  # type: ignore[misc]
                    async for token in async_iter:
                        yield token
                else:
                    # It's already an async iterator
                    async for token in stream_result:  # type: ignore[misc]
                        yield token
                return
            except Exception as e:
