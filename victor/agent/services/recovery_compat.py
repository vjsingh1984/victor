...
class StreamingRecoveryCoordinator:
    ...
    def _emit_async_event(
        ...
        *,
        ...
        """Emit an observability event without leaking coroutines on failure."""
        if self._event_bus is None:
            return
        try:
            loop = asyncio.get_running_loop()
        except RuntimeError:
            logger.debug("No event loop, skipping event emission for %s", topic)
            return
        ...
        try:
            ...
            emit_coro = self._event_bus.emit(**kwargs)
            loop.create_task(emit_coro)
        except Exception as e:
            if emit_coro is not None and hasattr(emit_coro, "close"):
                emit_coro.close()
