...
class ChatCoordinator:
    ...
    async def _chat_with_agentic_loop(self, user_message: str) -> CompletionResponse:
        ...
        # Enrich referential messages ("do it", "apply changes") with session context
        if orch._referential_intent_resolver:
            user_message = orch._referential_intent_resolver.enrich(user_message)
        ...
        while iteration < iteration_budget:
            ...
            # Assemble context-optimized message list
            assembled_messages = orch.messages
            if orch._context_assembler:
                assembled_messages = orch._context_assembler.assemble(
                    orch.messages,
                    max_context_chars=orch._conversation_controller.config.max_context_chars,
                    current_query=user_message,
                )
            ...
            if response.content:
                ...
                # Update session ledger from assistant response
                if orch._session_ledger and response.content:
                    orch._session_ledger.update_from_assistant_response(
                        response.content, turn_index=len(orch.messages)
                    )
