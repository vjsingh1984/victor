...
def _append_gate_failure_constraints(
    ...
    if gate_failures:
        sorted_failures = sorted(
            gate_failures.items(), key=lambda item: (-int(item[1]), item[0])
        )
...
def _append_planning_insights(
    ...
    forced_delta = _coerce_float(
        summary_metrics.get("planning_forced_slow_path_completion_delta")
    )
...
def _append_degradation_insights(
    ...
    provider_degradation_tasks = int(
        summary_metrics.get("provider_degradation_task_count", 0) or 0
    )
    content_degradation_tasks = int(
        summary_metrics.get("content_degradation_task_count", 0) or 0
    )
...
def _append_team_insights(
    ...
    materialized_count = int(
        summary_metrics.get("team_worktree_materialized_count", 0) or 0
    )
    ...
    merge_conflict_tasks = int(
        summary_metrics.get("team_merge_conflict_task_count", 0) or 0
    )
    out_of_scope_tasks = int(
        summary_metrics.get("team_out_of_scope_write_task_count", 0) or 0
    )
    readonly_violation_tasks = int(
        summary_metrics.get("team_readonly_violation_task_count", 0) or 0
    )
    cleanup_error_tasks = int(
        summary_metrics.get("team_cleanup_error_task_count", 0) or 0
    )
    ...
    if high_risk_count > 0 or merge_conflict_tasks > 0:
        ...
        insights.append(
            ExperimentInsight(
                ...
                confidence=min(
                    0.92, 0.45 + (0.08 * max(high_risk_count, merge_conflict_tasks))
                ),
                evidence={
    ...
    if (
        ...
        and low_risk_count >= materialized_count
        and cleanup_error_tasks == 0
    ):
    ...
    if (
        cleanup_error_tasks > 0
        or out_of_scope_tasks > 0
        or readonly_violation_tasks > 0
    ):
...
def _append_next_candidate(
    ...
    feasibility_rate = _coerce_float(
        summary_metrics.get("optimization_feasibility_rate")
    )
...
def _build_keywords(
    ...
    for task_summary in task_summaries:
        ...
        if task_summary.optimization:
            for failure_name in (
                task_summary.optimization.get("feasibility_failures") or []
            ):
                keywords.add(str(failure_name))
    for insight in insights:
        keywords.update(
            word for word in insight.summary.replace(".", " ").split() if "_" in word
        )
...
def analyze_evaluation_result(
    ...
    *,
    ...
    keywords = sorted(
        set(_build_keywords(scope, task_summaries, insights)) | keyword_seed
    )
    ...
    if runtime_feedback is not None:
        metadata["runtime_feedback_metadata"] = dict(
            runtime_feedback.get("metadata") or {}
        )
