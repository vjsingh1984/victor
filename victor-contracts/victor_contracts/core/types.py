...
class StageDefinition:
    ...
    def get_effective_tools(self, available_tools: List[str]) -> List[str]:
        ...
        effective.update(
            tool for tool in self.optional_tools if tool in available_tools
        )
...
def normalize_task_type_hints(
    ...
    return [
        normalize_task_type_hint(task_type, hint) for task_type, hint in hints.items()
    ]
...
def normalize_prompt_metadata(
    ...
    if isinstance(metadata, dict):
        return PromptMetadata(
            ...
            task_type_hints=normalize_task_type_hints(
                metadata.get("task_type_hints", {})
            ),
...
class TeamDefinition:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self, "members", normalize_team_member_definitions(list(self.members))
        )
...
class CapabilityRequirement:
    ...
    Attributes:
        capability_id: Stable capability identifier from `victor_contracts.constants`
...
def normalize_capability_requirements(
    ...
    return [
        normalize_capability_requirement(requirement) for requirement in requirements
    ]
...
class VerticalDefinition:
    ...
    def __post_init__(self) -> None:
        ...
        try:
            normalized_tier = (
                Tier(self.tier) if isinstance(self.tier, str) else self.tier
            )
            ...
            if not normalized_tools and normalized_tool_requirements:
                normalized_tools = [
                    ...
                    for requirement in normalized_tool_requirements
                ]
            ...
            raise VerticalConfigurationError(
                ...
                vertical_name=(
                    self.name if isinstance(self.name, str) and self.name else None
                ),
        ...
        object.__setattr__(
            self, "capability_requirements", normalized_capability_requirements
        )
        object.__setattr__(self, "tools", normalized_tools)
    ...
    def validate(self) -> None:
        ...
        if not isinstance(self.name, str) or not self.name.strip():
            raise VerticalConfigurationError(
                "Vertical definition name must be a non-empty string."
            )
        ...
        template_task_types = [
            template.task_type for template in self.prompt_metadata.templates
        ]
        ...
        hint_task_types = [
            hint.task_type for hint in self.prompt_metadata.task_type_hints
        ]
    ...
    def to_config(self) -> VerticalConfig:
        ...
        if self.capability_requirements:
            config_extensions["capability_requirements"] = (
                self.capability_requirements.copy()
            )
        if (
