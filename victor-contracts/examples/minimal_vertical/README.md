...
`victor_contracts` import surface from the `victor-contracts` distribution.

## Features

- ✅ **Zero Runtime Dependencies**: Only depends on `victor-contracts` (~1MB)
...
victor-minimal-vertical (imports victor_contracts)
        ↓
    victor-contracts (contract distribution)
        ↓
