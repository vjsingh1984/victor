...
def fix_file():
    ...
    for i in range(len(lines) - 5):
        ...
        if i == 771 and "orch.unified_tracker._progress.has_prompt_requirements = True" in lines[i]:
            lines[i] = (
                '        if orch.unified_tracker is not None and hasattr(orch.unified_tracker, "_progress"):\n'
        ...
        if (
            i == 775
            and "and prompt_requirements.tool_budget > orch.unified_tracker._progress.tool_budget"
            in lines[i]
        ):
        ...
        if (
            i == 777
            and "orch.unified_tracker.set_tool_budget(prompt_requirements.tool_budget)" in lines[i]
        ):
            lines[i] = (
                "            if orch.unified_tracker is not None:\n"
