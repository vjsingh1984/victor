...
async def refactoring_agent(code: str, goals: list[str]):
    ...
    agent = Agent.create(vertical="coding", tools=["read", "write"], temperature=0.4)

    goals_str = "\n".join(f"- {g}" for g in goals)

    result = await agent.run(f"""Refactor this code:
...
async def debugging_agent(code: str, error_message: str, context: str = ""):
...
async def documentation_agent(code_path: str, doc_style: str = "google"):
    ...
    agent = Agent.create(vertical="coding", tools=["read"], temperature=0.4)

    result = await agent.run(f"""Generate documentation for code at {code_path}.
...
async def code_explanation_agent(code: str, audience: str = "intermediate"):
    ...
    agent = Agent.create(vertical="coding", temperature=0.4)

    result = await agent.run(f"""Explain this code for a {audience} developer:
...
async def api_design_agent(requirements: str):
    ...
    agent = Agent.create(vertical="coding", temperature=0.5)

    result = await agent.run(f"""Design a REST API based on these requirements:
...
async def database_schema_agent(requirements: str):
    ...
    result = await agent.run(f"""Design a database schema:
...
async def microservice_design_agent(feature: str):
    ...
    result = await agent.run(f"""Design microservice architecture for: {feature}
...
async def code_migration_agent(code: str, source_lang: str, target_lang: str):
    """Migrate code from one language to another."""
    from victor import Agent

    agent = Agent.create(vertical="coding", temperature=0.3)

    result = await agent.run(f"""Migrate this code from {source_lang} to {target_lang}:
...
async def performance_optimization_agent(code: str, bottleneck: str = ""):
    ...
    agent = Agent.create(vertical="coding", tools=["python"], temperature=0.4)

    result = await agent.run(f"""Optimize this code for performance:
...
async def security_audit_agent(code: str):
    ...
    result = await agent.run(f"""Perform security audit of this code:
...
async def dependency_upgrade_agent(requirements_file: str, constraints: str = ""):
    """Plan dependency upgrades."""
    from victor import Agent

    agent = Agent.create(vertical="coding", tools=["read"], temperature=0.3)

    result = await agent.run(f"""Plan dependency upgrades for {requirements_file}.
...
async def ci_cd_pipeline_agent(project_type: str):
    ...
    agent = Agent.create(vertical="devops", temperature=0.3)

    result = await agent.run(f"""Generate CI/CD pipeline for {project_type} project.
...
async def kubernetes_manifest_agent(service_spec: str):
    ...
    result = await agent.run(f"""Generate Kubernetes manifests for:
...
async def terraform_agent(infrastructure: str):
    ...
    result = await agent.run(f"""Generate Terraform configuration for:
...
async def demo_coding_agents():
    ...
    print("1. Code Review:")
    result = await code_review_agent("def add(a, b): return a + b", "Python")
    print(result[:300] + "...\n")
