...
async def fact_check_agent(claims: list[str]):
    ...
    result = await agent.run(f"""Fact-check these claims:
...
async def academic_writing_agent(
    ...
    agent = Agent.create(vertical="research", temperature=0.5)

    result = await agent.run(f"""Help write a {paper_type} paper on: {topic}
...
async def plagiarism_check_agent(text: str):
    ...
    result = await agent.run(f"""Check this text for plagiarism:
...
async def research_methodology_agent(research_question: str):
    ...
    result = await agent.run(f"""Design research methodology for:
...
async def survey_design_agent(research_objectives: str):
    ...
    result = await agent.run(f"""Design a survey for:
...
async def interview_protocol_agent(research_topic: str):
    ...
    result = await agent.run(f"""Design interview protocol for: {research_topic}
...
async def data_analysis_plan_agent(research_questions: list[str], data_type: str):
    ...
    result = await agent.run(f"""Create data analysis plan for:
...
async def grant_proposal_agent(project_description: str, funding_agency: str):
    """Assist with grant proposal writing."""
    from victor import Agent

    agent = Agent.create(vertical="research", temperature=0.4)

    result = await agent.run(f"""Help write grant proposal for:
...
async def systematic_review_agent(research_question: str, databases: list[str]):
    ...
    result = await agent.run(f"""Design systematic review protocol for:
...
async def meta_analysis_agent(studies: list[dict]):
    ...
    result = await agent.run(f"""Design meta-analysis for {len(studies)} studies.
...
async def conference_abstract_agent(research_summary: str, conference: str):
    """Write conference abstract."""
    from victor import Agent

    agent = Agent.create(vertical="research", temperature=0.4)

    result = await agent.run(f"""Write conference abstract for:
...
async def poster_design_agent(research_content: str):
    ...
    result = await agent.run(f"""Design academic poster for:
...
async def presentation_script_agent(slides_content: str, duration_minutes: int):
...
async def peer_review_agent(paper_content: str):
    ...
    agent = Agent.create(vertical="research", temperature=0.3)

    result = await agent.run(f"""Conduct peer review of:
