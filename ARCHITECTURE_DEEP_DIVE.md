...
- Provides 32 enterprise-grade tools (git, testing, security, docker, etc.)
...
**Location**: `victor/agent/orchestrator.py:76`
...
- `self.tools`: ToolRegistry with all 32 tools
...
1. **MCP Server** (victor/mcp/server.py): Exposes Victor's tools to external clients
   ...
   - Other MCP clients can discover and call Victor's 32 tools
...
**Question**: Does Victor broadcast all 32 tools to the LLM every time?
...
2. Compare to pre-computed embeddings of all 32 tools
...
**The Problem**:
- If semantic selection returns 0 tools (low similarity), it sends ALL 32 tools
...
1. **MCP Server** (victor/mcp/server.py): Fully functional
   - Exposes Victor's 32 tools via MCP protocol
...
**Issue**: When semantic selection returns 0 tools, ALL 32 tools are sent.
...
- Intelligent tool selection (not broadcasting all 32 tools)
- Enterprise-grade tools (security scanning, CI/CD, testing, etc.)

**Current Limitations**:
- Tool selection can fallback to broadcasting all tools
- MCP not fully integrated with Ollama
